    ...
    if (!user.hasRoles(role)) {
      next(
        new UnauthorizedError(
          "User doesn't have the right role for this action"
        )
      );
    }
    next();
  ...
  return async (request: Request, response: Response, next) => {
    try {
      ...
      if (tmp) {
        ...
        next(
          new UnauthorizedError("User doesn't have permission for this action")
        );
      }
    } catch (error) {
      switch (error.constructor) {
        ...
        default:
          return next(
            new UnauthorizedError(
              "Error in policy class " +
                policyClassDecorator.name +
                ":" +
                policyName
            )
          );
