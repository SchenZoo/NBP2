...
export class FriendRequestController {
  ...
  public async getAll(
    ...
    return FriendRequestModel.find({ receiver: user.id })
      .sort({ createdAt: -1 })
      .paginate(query.skip, query.take);
  }

  @Get("/sent")
  public async getAllSentRequests(
    @QueryParams() query: Pagination,
    @CurrentUser() user: IUser
  ) {
    return FriendRequestModel.find({ sender: user.id })
      .sort({ createdAt: -1 })
      .paginate(query.skip, query.take);
  }
  ...
  public async create(@CurrentUser() user: IUser, @Param("id") id: string) {
    const friendRequest = await FriendRequestModel.findOne().or([
  ...
  public async acceptFriendRequest(
    ...
    if (friendRequest) {
      ...
      if (!friendship) {
        ...
        return new FriendshipModel({ mario: id, luigi: user.id }).save();
      } else {
        throw new ConflictError(
          "Bratici mario i luigi su vec braca prijatelji!"
        );
      }
    } else {
