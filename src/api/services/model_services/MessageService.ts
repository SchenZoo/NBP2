...
export class MessageService {
  ...
  async addMessage(
    ...
    let session = await this.chatSessionRepo.getSessionBetweenTwoUsers(
      sender.id,
      receiverId
    );
    if (chatMessage.filesBase64 && chatMessage.filesBase64.length) {
      chatMessage.files = await Promise.all(
        chatMessage.filesBase64.map((fileBase64) =>
          this.fileService.addBase64Image(
            fileBase64,
            ModelImagePath.PRIVATE_CHAT_MESSAGE
          )
        )
      );
    }
