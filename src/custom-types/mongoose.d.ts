interface IDocumentPaginated<T> {
  docs: T[];
  total: number;
  limit: number;
  offset: number;
...
declare module "mongoose" {
  ...
  type HookNextFunction = (error?: Error) => any;

  type HookDoneFunction = (error?: Error) => any;
  ...
  namespace SchemaTypeOpts {
    type DefaultFn<T> = (...args: any[]) => T;

    type RequiredFn<T> = (required: boolean, message?: string) => T;
    ...
    type ValidatorMessageFn = (props: ValidatorProps) => string;
    ...
    type AsyncPromiseValidationFn<T> = (value: T) => Promise<boolean>;
  ...
  > extends mquery {
    ...
    paginate(
      skip: number,
      limit: number
    ): DocumentQuery<IDocumentPaginated<DocType>, DocType, QueryHelpers> &
      QueryHelpers;
    /**
    ...
    lean<P = DocumentDefinition<DocType>>(
      bool?: boolean | object
    ): Query<T extends any[] ? P[] : P | null> & QueryHelpers;
  ...
  interface QueryFindOneAndUpdateOptions extends QueryFindOneAndRemoveOptions {
     ...
     */
    arrayFilters?: Array<{ [key: string]: any }>;
    /**
